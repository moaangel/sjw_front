<template>
  ...
  <div>
    ...
    <div class="d-flex gap-2">
      ...
      <router-link class="btn btn-primary" to="/user">돌아가기</router-link>
    </div>
  </div>
  <!-- <ModalView /> -->
   {{ placeData }}
...
const initMap = () => {
  ...
  map3.value = new kakao.maps.Map(container, options);
...
const searchPlaces = () => {
  if (!keyword.value.trim()) {
    alert('키워드를 입력해주세요!');
    return; // 키워드가 없으면 검색하지 않고 함수를 종료합니다.
  }
...
function displayPlaces(places) {
    ...
    for ( var i=0; i<places.length; i++ ) {
        ...
        (function(marker, title, addr, lat,lng) {
          kakao.maps.event.addListener(marker, 'click', function() {
              updateInputFields(title, addr, lat,lng);
            });
    ...
    map3.value.setBounds(bounds);
}

function updateInputFields(title, addr, lat,lng) {
  placeData.value.franname = title;
  placeData.value.addr = addr;
  placeData.value.latitude = lat;
  placeData.value.longitude = lng;
}
...
function addMarker(position, idx, title) {
    ...
    marker.setMap(map3.value); // 지도 위에 마커를 표출합니다
...
function displayInfowindow(marker, title) {
    ...
    infowindow.value.open(map3.value, marker);
}
...
const addUser = () => {
  let uData = {
    franname: placeData.value.franname,
